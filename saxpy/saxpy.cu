...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //Size of each buffer
    int size_of_buffer = N * sizeof(float);
    //Array for x
    cudaMalloc(&device_x, size_of_buffer);
    //Array for y
    cudaMalloc(&device_y, size_of_buffer);
    //Array for result
    cudaMalloc(&device_result, size_of_buffer);
    ...
    cudaMemcpy(resultarray, device_result, size_of_buffer, cudaMemcpyDeviceToHost);
